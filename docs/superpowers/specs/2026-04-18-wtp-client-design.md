...
**Operator observability for fail-closed recv branches.** The recv goroutine's fail-closed control-frame branches (`Goaway`, `ServerUpdate`, unknown frame types) MUST emit operator-distinguishable signals so that a reconnect storm caused by, e.g., the server issuing repeated `Goaway` frames is debuggable without log correlation. Each branch carries a dedicated `reason` string in its WARN log entry — those strings are STRUCTURED LOG FIELDS, not metric label values. Under the current `wtp_reconnects_total{reason}` schema (the seven labels enumerated in §"Metrics" below — `dial_failed | stream_recv_error | send_error | ack_timeout | heartbeat_timeout | server_goaway | unknown`), the three branches collapse onto two existing labels:

- **`Goaway`** — log WARN with `reason="goaway_received"` (structured-log field, NOT a metric label); the eventual reconnect drives `wtp_reconnects_total{reason="server_goaway"}` (existing label, no schema change). The recv goroutine surfaces an ordinary recv-error envelope on `errCh` carrying the `goaway` substring; the main goroutine's `recvErrCh` arm regresses to Connecting on the next select iteration.
- **`ServerUpdate`** — log WARN with `reason="server_update_unsupported_in_phase_4"` (structured-log field, NOT a metric label); the eventual reconnect drives `wtp_reconnects_total{reason="unknown"}` under the current schema. The dedicated `server_update_unsupported` label is the schema-expansion target tracked by **Task 22c** (see plan §"Task 22c: WTP reconnect-reason schema expansion for fail-closed control frames"). Same `errCh` envelope shape as `Goaway`.
- **Unknown frame types** — log WARN with `reason="recv_unknown_frame_type"` (structured-log field, NOT a metric label); the eventual reconnect drives `wtp_reconnects_total{reason="unknown"}` under the current schema. The dedicated `recv_unknown_frame` label is the schema-expansion target tracked by **Task 22c**. This is the proto-evolution defence: a future server may add control frames the client predates, and silently dropping them risks correctness.

All three categories are distinguishable from ordinary recv I/O failures (which drive `wtp_reconnects_total{reason="stream_recv_error"}`) and from each other via their dedicated `reason` strings in the structured WARN log; the current metric-label collapse to `server_goaway` / `unknown` is a known gap closed by Task 22c. The `errCh` envelope shape is currently a bare `error` whose `Error()` string carries the substring (`goaway`, `session_update`, or the unknown `%T` discriminator) — this is **temporary debugging guidance only**, suitable for diagnosing a specific incident by inspecting the recv-error log entry and correlating against the WARN log's `reason` field; it MUST NOT become the canonical operator contract because string substrings are brittle to refactors. The dedicated `wtp_reconnects_total{reason}` labels added by Task 22c are the operator-stable surface; treat any reliance on grepping the recv error string as transitional and remove it once Task 22c lands.
