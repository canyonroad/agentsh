...
**Concrete observables for span and surviving-count.** Neither quantity is currently emitted as a metric — both are derivable from on-disk WAL state and the existing structured-log surface, and a future `wtp_replay_plan_span` gauge (sampled at each reconnect, exposed in the `wtp_*` family per Task 22b's metrics expansion) would be a reasonable enhancement when the next round of operator visibility work lands. Today's path:

- `persistedAck.Generation` — read `meta.json` in the WAL directory: `cat $WAL_DIR/meta.json | jq .ack_high_watermark_gen` (the field name is `ack_high_watermark_gen` per `internal/store/watchtower/wal/meta.go` — the Meta struct's `AckHighWatermarkGen` field). Returns 0 when no ack has ever been recorded (paired with `ack_recorded == false` for disambiguation).
- `wal.HighGeneration()` — captured by the writer's structured-log surface on every generation roll (Task 12 contract): the WAL emits an INFO-level event keyed off the rotation that includes the new generation. Operators read these from the agent's stderr/log sink. There is no on-disk artifact that exposes `HighGeneration` directly without reading every segment header (segment files are named by index only — `internal/store/watchtower/wal/segment.go::segmentName(uint64)` — the generation lives inside `SegmentHeader.Generation`, not the filename).
- **Derived span:** subtract the two values above. A live span value also surfaces in the Anomaly WARN log family emitted by `ackSessionAck` / `ackBatchAck` / `ackServerHeartbeat` (per the §"True anomaly" sub-section above) — those records carry `local_persisted_gen` and `server_gen` fields, and the burst pattern `local_persisted_gen` repeatedly trailing `server_gen` by a wide margin is the same signal as `span > 10`, observable without any meta.json inspection.
- **Surviving-count** is harder: it requires reading every segment header (each header carries `Generation`) and counting distinct values. Operators MAY use a debug subcommand if one is built (none ships today); otherwise the metric-side proxy is the count of WAL quarantine events (`wtp_wal_quarantine_total`) holding steady — a stable surviving-count means GC is keeping pace with appends, which is the operationally-relevant invariant. A surviving-count gauge is also a reasonable Task 22b addition.
