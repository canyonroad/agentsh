...
- **Derived span:** subtract the two values above. A live span value also surfaces in the Anomaly WARN log family emitted by `ackSessionAck` (per the §"True anomaly" sub-section above) — those records carry `local_persisted_gen` and `server_gen` fields, and the burst pattern `local_persisted_gen` repeatedly trailing `server_gen` by a wide margin is the same signal as `span > 10`, observable without any meta.json inspection. **Coverage caveat:** `ackSessionAck` is the only emitter wired today; the analogous BatchAck and ServerHeartbeat handlers will land in Tasks 17/18/22 and will inherit the same WARN field schema. Until those tasks ship, only the SessionAck path on reconnect surfaces the anomaly fields, so an agent that holds a long Live session and never reconnects will produce no span observability through the WARN path.
