...
**Lossless integer parsing.** Conformance harnesses MUST parse `sequence`, `format_version`, and `generation` JSON numbers losslessly. Implementations whose default JSON parser does not preserve full uint64 precision (notably JavaScript / TypeScript using `JSON.parse`) MUST use a parser that returns string-form or arbitrary-precision integers (e.g., `JSON.parse` reviver returning BigInt, or a streaming parser that exposes raw token text). Validating against the `uint64_max_sequence` vector is the canonical gate for this requirement.

**Duplicate-key policy.** Conformance vector files MUST NOT contain duplicate JSON keys at any nesting level. Harnesses SHOULD reject duplicate keys explicitly (e.g., a strict JSON parser or post-decode unique-key check) and treat duplicate keys as a vector-file error, not as a "last value wins" silent acceptance. The published vector file is hand-authored and uniqueness is enforced by review and the strict decode contract (`DisallowUnknownFields` plus a duplicate-key check where the language standard library supports it).
