...
**Operator observability for fail-closed recv branches.** The recv goroutine's fail-closed control-frame branches (`Goaway`, `ServerUpdate`, unknown frame types) MUST emit operator-distinguishable signals so that a reconnect storm caused by, e.g., the server issuing repeated `Goaway` frames is debuggable without log correlation. Each branch carries a dedicated `reason` string in its WARN log entry AND will drive a dedicated `wtp_reconnects_total{reason}` label when the reconnect-counter plumbing lands in Tasks 18/19:

- **`Goaway`** — log WARN with `reason="goaway_received"`; the eventual reconnect drives `wtp_reconnects_total{reason="server_goaway"}`. The recv goroutine surfaces an ordinary recv-error envelope on `errCh` carrying the `goaway` substring; the main goroutine's `recvErrCh` arm regresses to Connecting on the next select iteration.
- **`ServerUpdate`** — log WARN with `reason="server_update_unsupported_in_phase_4"`; the eventual reconnect drives `wtp_reconnects_total{reason="server_update_unsupported"}`. Same `errCh` envelope shape as `Goaway`.
- **Unknown frame types** — log WARN with `reason="recv_unknown_frame_type"`; the eventual reconnect drives `wtp_reconnects_total{reason="recv_unknown_frame"}`. This is the proto-evolution defence: a future server may add control frames the client predates, and silently dropping them risks correctness.

All three categories MUST be distinguishable from ordinary recv I/O failures (which use `reconnect_reason="recv_io_error"` or similar) by their dedicated `reason` strings. This is a docs-only contract today: the WARN-emit and metric-label code lands when Tasks 18/19 wire the actual `wtp_reconnects_total{reason}` plumbing — until then, the `errCh` envelope is the only operator-facing surface and operators MUST grep the recv error string to distinguish the cases.
