...
- `wtp_dropped_invalid_frame_total{reason}` (counter labeled by frame-validation reason; increments when a schema-valid but semantically invalid protocol frame is dropped per the "Frame validation and forward compatibility" section above; reasons follow the same fixed-enum + escapeLabelValue pattern as `wtp_session_init_failures_total{reason}`. Enumerated reasons today: `event_batch_body_unset`, `event_batch_compression_unspecified`, `session_init_algorithm_unspecified`, plus `unknown` as the catch-all. Structured log emitted with fields {session_id, reason, err}; chain/sequence are NOT logged because frame-validation failures occur before per-record drop bookkeeping)
