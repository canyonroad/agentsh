...
**Operator observability for fail-closed recv branches.** The recv goroutine's fail-closed control-frame branches (`Goaway`, `ServerUpdate`, unknown frame types) MUST emit operator-distinguishable signals so that a reconnect storm caused by, e.g., the server issuing repeated `Goaway` frames is debuggable without log correlation. The current state at commit `0b28f74e` and the future structured-log contract are explicitly distinguished below.

**Current state (commit `0b28f74e`).** The recv multiplexer in `internal/store/watchtower/transport/recv_multiplexer.go` (the three branches near lines 222–250) pushes plain Go errors onto `rs.errCh` for `Goaway`, `ServerUpdate`, and unknown frame types. The error string is the only operator-visible signal today — there is NO `slog.Warn(...)` call on these branches yet, NO structured `reason` field, and NO WARN-level log entry the operator can grep for. The transport's reconnect attempt that follows surfaces under the existing `wtp_reconnects_total{reason}` labels (the seven labels enumerated in §"Metrics" below — `dial_failed | stream_recv_error | send_error | ack_timeout | heartbeat_timeout | server_goaway | unknown`); the three fail-closed branches collapse onto two of those labels today:

- **`Goaway`** — drives `wtp_reconnects_total{reason="server_goaway"}` (existing label). The recv goroutine surfaces an ordinary recv-error envelope on `errCh` carrying the `goaway` substring (literal: `recv: control frame goaway not yet handled`); the main goroutine's `recvErrCh` arm regresses to Connecting on the next select iteration.
- **`ServerUpdate`** — drives `wtp_reconnects_total{reason="unknown"}` under the current seven-label schema. `errCh` envelope carries the `session_update` substring (literal: `recv: control frame session_update not yet handled`).
- **Unknown frame types** — drives `wtp_reconnects_total{reason="unknown"}` under the current seven-label schema. `errCh` envelope carries the unknown `%T` discriminator (literal: `recv: unknown control frame %T, returning to Connecting`). This is the proto-evolution defence: a future server may add control frames the client predates, and silently dropping them risks correctness.

Until both Task 22c (metric-label expansion) AND Task 22d (structured WARN logging) land, operators distinguish branches via the `errCh` substring or by inspecting the proto frame type at the transport diagnostic layer. This `errCh`-substring guidance is **transitional debugging only**, suitable for one-off incident triage; it MUST NOT become the canonical operator contract because string substrings are brittle to refactors.

**Future contract (owned by Tasks 22c + 22d).** The end-state operator surface is two-pronged:
...
- The `wtp_reconnects_total{reason}` schema will gain dedicated labels `server_update_unsupported` and `recv_unknown_frame` so the metric-label collapse described above goes away. Owned by **Task 22c** (see plan §"Task 22c: WTP reconnect-reason schema expansion for fail-closed control frames"). The `Goaway` branch keeps its existing `server_goaway` label across this transition.

Once both tasks land, the canonical operator surface is the dedicated metric labels for the rate signal AND the structured WARN log's `reason` field for the per-branch discriminator; the transitional `errCh` substring guidance is then removed. All three categories will be distinguishable from ordinary recv I/O failures (which drive `wtp_reconnects_total{reason="stream_recv_error"}`) and from each other via both surfaces.

**Lifecycle states for reconnect-reason observability.** This subsection's operator contract evolves through three named states; the spec wording you are reading currently describes state 1.

1. **Schema landed, emitter not wired** — the new metric labels (`server_update_unsupported`, `recv_unknown_frame`) appear in `/metrics` at zero on registration via the always-emit contract, but no code path increments them yet AND no structured WARN log carries the `reason` field. This is the state that Task 22c Steps 1–3 (metric-schema-only) leaves the system in for `server_update_unsupported` and `recv_unknown_frame`. The interim spec wording (current state described above, three branches collapsing onto `server_goaway` / `unknown`) STAYS IN EFFECT throughout this state; operators read the existing labels and the bare `errCh` error string for diagnostics.
2. **Emitter wired** — the `IncReconnects(server_update_unsupported)` / `IncReconnects(recv_unknown_frame)` call sites land in transport (in Tasks 18 / 19, paired with the actual reconnect plumbing) AND `slog.Warn(... reason=...)` calls land in the three recv-multiplexer branches (Task 22d). Operators can now filter by the dedicated metric labels and grep the structured WARN log's `reason` field. The interim spec wording is now stale-but-correct (it under-describes what the system actually emits); the spec rewrite to end-state is gated on this state being reached, see state 3.
3. **End-state spec wording active** — Task 22c Step 4 promotes this subsection to describe the dedicated labels as the live operator surface and removes the transitional `errCh`-substring guidance. Step 4 MUST NOT run until both Tasks 18 and 19 (emitter call sites) and Task 22d (WARN logging) have landed; otherwise the spec would describe operator-visible labels as "live" when they are actually zero-valued series with no producer and the WARN field as queryable when no log line carries it.
