...
- **`ServerUpdate`** — Future Tasks 18/19 will wire `metrics.IncReconnects(...)` so this branch drives `wtp_reconnects_total{reason="unknown"}` under the current seven-label schema (and `wtp_reconnects_total{reason="server_update_unsupported"}` after Task 22c). Until then, only the bare error string distinguishes it. `errCh` envelope today carries the `session_update` substring (literal: `recv: control frame session_update not yet handled`).
- **Unknown frame types** — Future Tasks 18/19 will wire `metrics.IncReconnects(...)` so this branch drives `wtp_reconnects_total{reason="unknown"}` under the current seven-label schema (and `wtp_reconnects_total{reason="recv_unknown_frame"}` after Task 22c). Until then, only the bare error string distinguishes it. `errCh` envelope today carries the unknown `%T` discriminator (literal: `recv: unknown control frame %T, returning to Connecting`). This is the proto-evolution defence: a future server may add control frames the client predates, and silently dropping them risks correctness.
...
**Lifecycle states for reconnect-reason observability.** This subsection's operator contract evolves through four named states; the spec wording you are reading currently describes state 0 (the actual code reality at commit `7daa69eb`).
