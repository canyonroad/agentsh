...
- `wtp_dropped_missing_chain_total` (counter; increments when `compact.Encode` returns `ErrMissingChain` because the composite store did not stamp `ev.Chain` — record is dropped at the AppendEvent boundary, structured log emitted with fields {session_id, err}; `sequence`/`generation` are intentionally omitted because the chain is unstamped and dereferencing would panic)
...
Dashboard/alerting impact. The new sink-failure metrics — six unlabeled counters (`wtp_dropped_missing_chain_total`, `wtp_dropped_invalid_mapper_total`, `wtp_dropped_invalid_timestamp_total`, `wtp_dropped_mapper_failure_total`, `wtp_dropped_invalid_utf8_total`, `wtp_dropped_sequence_overflow_total`) plus two labeled families (`wtp_session_init_failures_total{reason}`, `wtp_session_rotation_failures_total{reason}`) — all follow the always-emit contract: the families appear at zero on every scrape regardless of activity, so adding them does not change cardinality at quiescence and does not require a phased rollout. Existing dashboards keep working unchanged; new alerting rules can be added at operator discretion. Suggested alerting:
...
- Alert-not-page on `rate(wtp_dropped_missing_chain_total[5m]) > 0` — composite store failed to stamp `ev.Chain` before fanout (composite-store regression).
...
Encode failures in the WTP sink's `AppendEvent` path are classified via `errors.Is` and dropped (chain does NOT advance). Each error class has a distinct counter (`wtp_dropped_missing_chain_total`, `wtp_dropped_invalid_mapper_total`, `wtp_dropped_invalid_timestamp_total`, `wtp_dropped_mapper_failure_total`). The drops are sink-internal; they do NOT propagate as session-fatal errors. See Task 22a for the metric definitions and Task 23 for the sink-side `errors.Is` wiring.

All `compact.Encode` drop classes follow the same boundary contract as the existing per-record drops `wtp_dropped_invalid_utf8_total` (see "per-record encodes only" paragraph above) and `wtp_dropped_sequence_overflow_total` (see sequence-width contract paragraph above):
...
- **Log**: structured WARN log emitted with fields `{session_id, sequence, generation, err}`. The `err` value is the wrapped error returned by `Encode`; its message identifies the specific failure (e.g. `compact: invalid mapper`, `compact mapper: <underlying>`). The `ErrMissingChain` case omits `sequence` and `generation` because `ev.Chain` is unstamped and dereferencing would panic.
- **AppendEvent return**: returns `nil` (sink-internal drop, not propagated to the caller — matches the existing `invalid_utf8` and `sequence_overflow` patterns above). The metric and the structured log are the only signals operators see.

The `default` branch of the `errors.Is` switch — i.e., `Encode` returned an error that is not one of the three sentinels — corresponds to a mapper-side failure wrapped by `Encode` as `compact mapper: %w`. That branch increments `wtp_dropped_mapper_failure_total`. There is no fifth sentinel for this case because the underlying error originates inside the mapper implementation; operators classify it from the wrapped `err` text in the structured log.
