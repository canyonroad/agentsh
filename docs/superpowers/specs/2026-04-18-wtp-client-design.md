...
- **Higher generation with local WAL having emitted records in that generation (`server_gen > local persistedAck.gen` AND `server_gen <= wal.WrittenHighGeneration()`)**: ADVANCE BOTH cursors. Call `wal.MarkAcked(serverGen, serverSeq)`. This is the normal cross-generation ack-advance path: the client's writer rolled to `server_gen` (emitting records), the client sent those records, and the server has now durably acked through `(server_gen, server_seq)` — but only advanced `persistedAck` on disk through the previous generation. The WAL's `MarkAcked` already supports monotonic cross-generation advancement (see `wal.MarkAcked`'s `advance` predicate — `gen > w.ackHighGen || (gen == w.ackHighGen && seq > w.ackHighSeq)`). No anomaly, no WARN; this is a legitimate restart/reconnect case. The clamp helper still guards against fabricated claims in this case by requiring `server_seq <= wal.HighWaterSequence()` when the server's generation matches the WAL's CURRENT generation (i.e. `wal.WrittenHighGeneration()`); if the server's generation is STRICTLY less than the WAL's current generation, the server-seq guard is not enforceable by `wal.HighWaterSequence()` alone (the accessor only reports the current-generation tip), so the helper admits the advance without the guard. Operators can still detect a fabricated-future-seq-in-closed-generation case via the server's own ack-consistency checks; the client's WAL has already moved past that generation and no loss surface is opened by admitting the advance.
...
- **Higher generation with WAL never having emitted records in that generation (`server_gen > wal.WrittenHighGeneration()`)**: take the Anomaly path with `AnomalyReason="future_generation"`. NEITHER cursor moves; `wal.MarkAcked` is NOT called. Only the client's writer ever rolls generations, so a server tuple in a generation the local WAL has never written cannot be legitimate.

**True anomaly (every case here produces a rate-limited WARN; local cursors are kept unchanged in all sub-cases).** Under the two-cursor model the anomaly bucket covers three disjoint shapes; all of them KEEP local cursors unchanged and emit a rate-limited WARN with FULL tuple context (`server_gen`, `server_seq`, `local_persisted_gen`, `local_persisted_seq`, `wal_high_water_seq`, `wal_written_high_gen`).

1. **`serverGen == local persistedAck.gen` AND `serverSeq > wal.HighWaterSequence()`** — same generation, sequence beyond anything the WAL has ever appended in this generation. The server is ahead of physically-emitted history; this is the classic anomaly shape and is most often a test/replay misconfiguration or a server-side state corruption.
...
3. **`serverGen > wal.WrittenHighGeneration()`** — server claims a generation the local WAL has not yet rolled to. Only the writer ever rolls generations, so the claim is fabricated relative to local history. Increment `wtp_anomalous_ack_total{reason="future_generation"}`. NOTE: a higher-generation server tuple where `server_gen <= wal.WrittenHighGeneration()` is NOT an anomaly — it is the legitimate Adopted case described above (the client wrote records in `server_gen` and the server is now acking them).
...
**Loss between replay cursor and persisted ack.** When the server presents a lex-lower tuple **with the same generation as `persistedAck`** AND the WAL has already GC'd the segments holding records `(remoteReplayCursor, persistedAck]` (because GC was driven by an earlier higher ack), the data the server is asking for is permanently gone. The transport detects this BEFORE opening the replay reader: it queries `wal.WAL.EarliestDataSequence()` (a new accessor introduced in Task 14a — returns the lowest user sequence still on disk) and computes a synthetic loss range as follows. Let `gapStart = remoteReplayCursor.seq + 1`:

- If `EarliestDataSequence` returns `(gen, earliest, true)` AND `gen == replayCursor.gen` AND `earliest > gapStart`: the gap `[gapStart, earliest-1]` has been GC'd. `gapEnd = earliest - 1`. Replay reader opens at `earliest`.
- If `EarliestDataSequence` returns `(_, _, false)` (no data on disk in the current generation) AND `gapStart <= persistedAck.seq` (i.e. `replayCursor.seq < persistedAck.seq`): the entire range `[gapStart, persistedAck.seq]` is unrecoverable because the WAL has GC'd everything down to (or past) the persisted ack watermark and there is no surviving data. `gapEnd = persistedAck.seq`. Replay reader opens at `persistedAck.seq + 1` (which yields no records — the cursor is at the WAL's tip with nothing to replay this generation). This is the new "fully-GC'd, server-regressed-below-persisted" path that the round-9 design silently dropped; round-10 surfaces it as `ack_regression_after_gc`.
- If `EarliestDataSequence` returns `(_, _, false)` AND `gapStart > persistedAck.seq`: no loss to emit (the server has not regressed below what we've persisted; the WAL is empty because everything has been acked-and-GC'd).
- If `EarliestDataSequence` returns `(gen, earliest, true)` AND `earliest <= gapStart`: no loss to emit (the server's tuple is still on disk).

When the loss range is non-empty, the transport synthesizes a single `wal.LossRecord{FromSequence: gapStart, ToSequence: gapEnd, Generation: persistedAck.gen, Reason: "ack_regression_after_gc"}` **in memory** and passes it to the Replayer via `Replayer.PrefixLoss` (or equivalent constructor option) BEFORE the Replayer opens its first batch. The Replayer's first batch surfaces this loss marker as the first record so it reaches the server through the same `TransportLoss` propagation path the existing CRC-corruption / overflow markers use (per Task 13). The synthetic loss is **NOT persisted to the WAL** — `wal.AppendLoss` writes to the live tail (out of order vs. the surviving on-disk data) and latches the WAL into fatal on I/O failure. Neither failure mode is acceptable for a per-reconnect transient signal: in-memory synthesis keeps the loss marker ordered correctly relative to the replay window AND avoids the WAL fatal-latch surface entirely. Increment `wtp_ack_regression_loss_total` (a counter — see Task 22b). This is permanent loss — the gap is unrecoverable from local state — but the protocol surface (`TransportLoss`) makes it visible to operators rather than silently swallowing the records.
