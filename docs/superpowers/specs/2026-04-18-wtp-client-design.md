...
2. **`remoteReplayCursor` (gen, seq)** — the *server-belief* cursor. Where the server thinks its ack watermark sits. May be lex-LOWER than `persistedAck` if the server (gradual rollout or partition recovery) presents a stale tuple. Drives the **Replaying state's reader-start calculation**: the replay reader opens at `remoteReplayCursor + 1` (NOT `persistedAck + 1`) so the gap between the cursors is re-sent. Lives in `t.remoteReplayCursor (gen, seq)` on Transport.
...
- **First apply (no prior `persistedAck` recorded, i.e., `t.persistedAckPresent == false`)**: ADOPT the server tuple wholesale into BOTH `persistedAck` AND `remoteReplayCursor`. Call `wal.MarkAcked(serverGen, serverSeq)`. No anomaly log, no WARN — first contact between client and server cannot be classified as anomalous because there is no local baseline.
- **`(server_gen, server_seq) > (local persistedAck.gen, local persistedAck.seq)` lex**: ADVANCE BOTH cursors. Call `wal.MarkAcked(serverGen, serverSeq)`. This is the normal, healthy ack-advance path. (The previous design's "anomalous higher" framing has been retired — under the two-cursor model a server claiming to have acked records the client genuinely produced is the everyday case. The only true anomaly is the server claiming an ack BEYOND any sequence the WAL has ever appended in the current generation; see "True anomaly" below.)
- **`(server_gen, server_seq) < (local persistedAck.gen, local persistedAck.seq)` lex**: SET `remoteReplayCursor = (serverGen, serverSeq)`; LEAVE `persistedAck` UNCHANGED. Do NOT call `wal.MarkAcked`. This is the legitimate stale-watermark recovery path. The next replay window opens at `serverSeq + 1` and re-sends the gap to the server. Log INFO (NOT WARN) with both tuples (`server_gen`, `server_seq`, `local_persisted_gen`, `local_persisted_seq`) — this is normal recovery, not an anomaly. The persistedAck stays put because the WAL has already GC'd or may GC at any time the segments at that watermark; the in-memory mirror MUST stay in lock-step with the on-disk meta.json so a future restart re-seeds correctly.
- **Equal**: no-op on both cursors.

**True anomaly (the only case that produces a WARN).** A server claim of `(serverGen, serverSeq)` such that `serverGen` matches the WAL's current generation AND `serverSeq > w.highSeq` (the WAL's high-water sequence at this moment) — i.e., the server is acknowledging a record the WAL has never produced. Under the two-cursor model this is the only logically anomalous case (the server is ahead of physically-emitted history). The client KEEPS local cursors unchanged and emits a rate-limited WARN with FULL tuple context (`server_gen`, `server_seq`, `local_persisted_gen`, `local_persisted_seq`, `wal_high_water_seq`). Most production deployments will never hit this case; if they do, it indicates a server-side state corruption or test/replay misconfiguration.

**Loss between replay cursor and persisted ack.** When the server presents a lex-lower tuple AND the WAL has already GC'd the segments holding records `(remoteReplayCursor, persistedAck]` (because GC was driven by an earlier higher ack), the data the server is asking for is permanently gone. Per the existing WAL loss-marker design (Task 13), the replayer encounters loss markers in the Reader stream and propagates a `TransportLoss{from: remoteReplayCursor+1, to: persistedAck, reason: "ack_regression_after_gc"}` frame to the server. This is permanent loss — the gap is unrecoverable from local state — but the protocol surface (TransportLoss) makes it visible to operators rather than silently swallowing the records.
