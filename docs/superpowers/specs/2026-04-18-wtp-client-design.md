...
2. Increment `wtp_dropped_invalid_frame_total{reason}` with the appropriate frame-validation reason. The enumerated values are `event_batch_body_unset`, `event_batch_compression_unspecified`, `event_batch_compression_mismatch` (uncompressed body declares non-NONE compression, or compressed_payload body declares NONE), `session_init_algorithm_unspecified`, `payload_too_large` (compressed_payload exceeds `MaxCompressedPayloadBytes`, matches `wtpv1.ErrPayloadTooLarge`), and `decompress_error` (streaming decompression fails or exceeds `MaxDecompressedBatchBytes`), plus `unknown` as a forward-compat catch-all for any frame validation failure not yet covered by the enumerated reasons. The label key is `reason` to match the existing `wtp_session_failures_total{reason}` / `wtp_reconnects_total{reason}` convention. The catch-all is intentional: a non-zero `wtp_dropped_invalid_frame_total{reason="unknown"}` series MUST be treated as an operator-visible signal that a new validator failure class has shipped without being added to the enum; the next maintenance cycle MUST extend the enum (and the matching `wtpv1.ValidationReason` constants — see "Frame validation and forward compatibility" implementation notes) to classify it under a dedicated label.

   **Reason classification (fallback rule).** Receivers MUST attempt `errors.As(err, &ve)` against `*wtpv1.ValidationError` to extract the typed `ve.Reason`. If `errors.As` returns false (i.e., the validator returned a plain `fmt.Errorf("%w: ...", wtpv1.ErrInvalidFrame, ...)` not wrapped in `*ValidationError` — for example, the unknown-oneof default branch in `ValidateEventBatch` that intentionally stays a bare `fmt.Errorf` so a future protobuf revision adding a new oneof arm without updating the switch surfaces here), receivers MUST classify it as `reason="unknown"` and increment `wtp_dropped_invalid_frame_total{reason="unknown"}`. Receivers MUST NOT inspect `err.Error()` or any substring of the formatted message to derive a reason — the typed boundary is the contract; the formatted message embeds peer-supplied byte counts and oneof discriminators that violate the sanitization rule. The validator MAY emit plain `ErrInvalidFrame` for forward-compat cases (new oneof discriminators added to the proto schema before the validator is updated, future schema fields not yet covered by `ValidationReason`) — these are explicitly the `unknown` reason class and reaching that branch is the operator-visible signal that the enum needs extending.
...
Sanitization applies to ALL log emission paths, not just the structured drop-class record at the receiver. Transport-layer interceptors (gRPC unary/stream interceptors, panic-recovery handlers, debug-trace formatters) and any other code that touches a `*wtpv1.ValidationError` or its `Inner` error MUST log only the `Reason` field. The `Inner` error MUST NOT be passed to any logger that emits to production sinks because its formatted message embeds the same peer-supplied byte counts and oneof discriminators the structured-log rule excludes; routing it through a different logger does not change the threat model.

Implementations SHOULD make this enforceable at the type level by giving `*ValidationError` an `Error() string` method that returns ONLY the canonical reason value (no peer-derived content) — this means even a naive `slog.Error("...", "err", ve)` or `fmt.Sprintf("%s", ve)` cannot leak peer bytes. The `Inner` error remains accessible via `Unwrap()` for in-memory inspection during tests (and for `errors.Is` / `errors.As` chain traversal), but is never serialized to logs. The TDD test for `ValidationError.Error()` MUST assert e.g. `(&ValidationError{Reason: ReasonPayloadTooLarge, Inner: fmt.Errorf("32MiB exceeds 8MiB cap")}).Error() == "payload_too_large"` to lock the behavior in.
...
- `wtp_dropped_invalid_frame_total{reason}` (counter labeled by frame-validation reason; increments when a schema-valid but semantically invalid protocol frame is dropped per the "Frame validation and forward compatibility" section above; reasons follow the same fixed-enum + escapeLabelValue pattern as `wtp_session_init_failures_total{reason}`. Enumerated reasons today: `event_batch_body_unset`, `event_batch_compression_unspecified`, `session_init_algorithm_unspecified`, `event_batch_compression_mismatch` (uncompressed body declares non-NONE compression, or compressed_payload body declares NONE), `payload_too_large` (compressed_payload exceeds `MaxCompressedPayloadBytes`, matches `wtpv1.ErrPayloadTooLarge`), and `decompress_error` (streaming decompression fails or exceeds `MaxDecompressedBatchBytes` — enforced downstream of the validator but classified under this reason by the receiver), plus `unknown` as the catch-all. The catch-all is intentional and acts as a forward-compat safety net: a non-zero `wtp_dropped_invalid_frame_total{reason="unknown"}` series is operator-visible signal that a new validator failure class has shipped without being added to the enum, and the next maintenance cycle MUST extend the enum to cover it. Structured log emitted with fields {session_id, reason, hex_prefix} where `session_id` is the local session UUID (NOT peer-supplied), `reason` is the canonical enum value (NOT peer-supplied), and `hex_prefix` is a hex-encoded prefix of the offending frame's serialized representation capped at 16 input bytes (so 32 hex chars output) per the "Invalid-frame log sanitization" rule. The raw `err` returned by `wtpv1.ValidateEventBatch` / `ValidateSessionInit` MUST NOT be logged because its formatted message embeds peer-supplied byte counts and oneof discriminators that violate the sanitization rule; if implementers need debug visibility for a specific failure class they SHOULD bump the validator log level locally during diagnosis, not change the production field set. Chain/sequence are NOT logged because frame-validation failures occur before per-record drop bookkeeping.)
