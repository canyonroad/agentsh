...
func TestMountFUSEForSession_RegistersMountPointNotSourcePath(t *testing.T) {
	...
	if !reg.IsUnderFUSEMount(s.ID, mountPoint) {
		t.Fatalf("expected mount point %q to be registered in MountRegistry", mountPoint)
	}
	...
	if reg.IsUnderFUSEMount(s.ID, s.Workspace) {
		t.Fatalf("source path %q should NOT be registered in MountRegistry (only mount points)", s.Workspace)
	}
	...
	if reg.IsUnderFUSEMount(s.ID, mountPoint) {
		t.Fatalf("mount point %q should be deregistered after unmount", mountPoint)
	}
}
