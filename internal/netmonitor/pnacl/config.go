...
func mergeChildConfigs(base, override []ChildConfig) []ChildConfig {
	...
	for _, overrideCC := range override {
		if baseCC, exists := baseChildren[overrideCC.Name]; exists {
			...
			// Override inherit flag - note: YAML will set false if not present
			// so we check if rules are present to determine if this is an override
			if len(overrideCC.Rules) > 0 || overrideCC.Inherit != baseCC.Inherit {
